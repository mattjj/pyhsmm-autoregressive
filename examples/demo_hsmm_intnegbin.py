...
from pyhsmm.basic.distributions import NegativeBinomialIntegerRDuration
...
model = m.ARWeakLimitHDPHSMMIntNegBin(
        ...
        obs_distns=[d.AR_MNIW(nu_0=3,S_0=np.eye(2),M_0=np.zeros((2,4)),Kinv_0=np.eye(4))
            for state in range(Nmax)],
        dur_distns=[NegativeBinomialIntegerRDuration(np.ones(10),alpha_0=10,beta_0=1)
            for state in range(Nmax)]
        )
