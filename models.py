...
class ARHMM(pyhsmm.models.HMM):
    ...
    def add_data(self,data,stateseq=None):
        ...
        self.states_list.append(ARHMMStates(data.shape[0],self.state_dim,self.obs_distns,self.dur_distns,self.trans_distn,self.init_state_distn,trunc=self.trunc,data=strided_data,stateseq=stateseq,nlags=self.nlags))
...
class ARHSMM(pyhsmm.models.HSMM):
    ...
    def add_data(self,data,stateseq=None,censoring=None):
        strided_data = AR_striding(data,self.nlags)
        self.states_list.append(ARHSMMStates(data.shape[0],self.state_dim,self.obs_distns,self.dur_distns,self.trans_distn,self.init_state_distn,trunc=self.trunc,data=strided_data,stateseq=stateseq,censoring=censoring,nlags=self.nlags))
