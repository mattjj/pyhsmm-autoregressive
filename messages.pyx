...
cdef extern from "messages.h":
    cdef cppclass dummy[Type]:
        ...
        Type resample_featureregressionhmm(
...
# NOTE: this code is the same as the above except it calls a different function
# in the C++ header file and it requires the D parameter to be passed in.
# TODO reduce code repetition
def resample_featureregressionhmm(
        integral D,
        list pi_0s,
        list As,
        floating[:,:,::1] params,
    ...
    cdef int K = len(datas)        # number of sequences
    cdef bool affine = params.shape[2] % D
    cdef int nfeat = params.shape[2] - (affine + D)
    cdef int32_t[::1] Ts = np.array([d.shape[0] for d in datas]).astype('int32')
    ...
    with nogil:
        for j in prange(K+1):
            if j != 0:
                i = j-1
                likes[i] = ref.resample_featureregressionhmm(
                        M,Ts[i],D,nfeat,affine,
                        pi_0s_v[i],As_v[i],
                        &params[0,0,0],&normalizers[0],
                        datas_v[i],
                        &stats[2*i,0,0,0],&ns[2*i,0],&transcounts[2*i,0,0],
